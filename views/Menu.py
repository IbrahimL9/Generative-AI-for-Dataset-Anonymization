...
class Menu(QListWidget):
    ...
    def __init__(self):
        ...
        # Définition des pages et leurs indices
        self.page_mapping = {
            "Open file": 0,
            "Display": 1,
            "Inspect": 2,
        ...
        # Ajout des sections et des sous-éléments
        for section, icon_path, sub_items in self.pages:
            ...
            for sub_item in sub_items:
                sub_item_widget = QListWidgetItem(f"    • {sub_item}")  # Ajout de la puce (•)
        ...
        # Afficher par défaut le sous-menu "Source" et sélectionner "Open file"
    ...
    def addSpacingItem(self,count=4):
        """ Ajoute un espace vide entre les sections. """
    ...
    def on_page_changed(self, index):
        """ Gère l'affichage des sous-menus et la sélection de la page. """
        ...
        if item in self.section_items:
            ...
            text = item.text().strip().replace("• ", "")  # Enlever la puce (•) pour correspondre au mapping
    ...
    def show_initial_submenu(self, section_name, sub_item_name):
        """ Affiche les sous-éléments de la section sélectionnée et met en surbrillance l'élément actif. """
        for sec, sub_items in self.sub_items.items():
            for sub_item in sub_items:
                sub_item.setHidden(sec != section_name)
